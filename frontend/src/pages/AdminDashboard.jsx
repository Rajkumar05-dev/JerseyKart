...
import { CheckCircle2, ImageUp, Package, Plus, Trash2, Pencil, LogOut, Store, ShoppingCart, ChevronDown } from 'lucide-react';
...
const AdminDashboard = () => {
  ...
  const fetchUserOrderStats = async () => {
    ...
    setError('');
    try {
      const { data } = await api.get('/api/admin/orders/user-statistics');
      setUserStats(data);
    } catch (err) {
      const details = formatErrorDetails(err);
      setError(`Failed to load user order statistics: ${details}`);
      addErrorLog('Load user orders', err);
    } finally {
  ...
  useEffect(() => {
    if (activeTab === 'orders' && userStats.length === 0) {
      fetchUserOrderStats();
    }
  }, [activeTab]);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-10">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 mb-8">
        ...
        <div className="glass-card p-5">
          <p className="text-sm text-gray-500">Total Users (with orders)</p>
          <p className="text-3xl font-bold text-primary mt-1">{userStats.length}</p>
        </div>
        <div className="glass-card p-5">
      ...
      <div className="mb-8 flex gap-2 border-b dark:border-gray-700">
        <button
        ...
        >
          <div className="flex items-center gap-2">
            <ShoppingCart size={18} /> User Orders
          </div>
        </button>
      </div>
      ...
      {activeTab === 'orders' && (
        <div className="glass-card p-5 mb-8">
          <h2 className="font-bold dark:text-white mb-4 flex items-center gap-2">
            <ShoppingCart size={20} /> User Order Statistics
          </h2>
          {loadingOrders ? (
            <p className="text-center text-gray-500 py-8">Loading user order statistics...</p>
          ) : userStats.length === 0 ? (
            <p className="text-center text-gray-500 py-8">No users with orders yet.</p>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm">
                ...
                <tbody>
                  {userStats.map((stat) => (
                    <motion.tr key={stat.userId} className="border-b dark:border-gray-800 dark:text-white">
                      <td className="p-4 font-medium">
                        {stat.firstName} {stat.lastName}
                      </td>
                      <td className="p-4 text-sm truncate">{stat.email}</td>
                      <td className="p-4 text-sm">{stat.mobile || 'N/A'}</td>
                      <td className="p-4">
                        <span className="inline-flex rounded-full px-3 py-1 text-xs font-semibold bg-blue-50 text-blue-600 dark:bg-blue-900/20 dark:text-blue-300">
                          {stat.totalOrders}
                        </span>
                      </td>
                      <td className="p-4">
                        <button
                          ...
                          className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg border dark:border-gray-700 dark:text-white hover:bg-gray-50 dark:hover:bg-gray-800 transition"
                        >
                          <span className="text-xs font-medium">Details</span>
                          <ChevronDown size={14} className={`transition-transform ${expandedUser === stat.userId ? 'rotate-180' : ''}`} />
                        </button>
                      </td>
                    </motion.tr>
                  ))}
          ...
          {expandedUser && (
            <motion.div
              ...
              className="mt-6 p-4 rounded-lg border border-blue-200 dark:border-blue-900/30 bg-blue-50 dark:bg-blue-900/10"
            >
              <div className="flex justify-between items-center mb-4">
                <h3 className="font-bold dark:text-white">
                  {userStats.find((s) => s.userId === expandedUser)?.firstName}'s Orders
                </h3>
                <button
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left text-sm">
                  ...
                  <tbody>
                    {userStats
                      .find((s) => s.userId === expandedUser)
                      ?.orderDetails.map((order) => (
                        <tr key={order.orderId} className="border-b dark:border-gray-700">
                          ...
                          <td className="p-3 font-medium">₹{Math.round(order.totalPrice)}</td>
                          <td className="p-3 text-xs">{order.orderDate}</td>
                        </tr>
                      ))}
                  </tbody>
                </table>
